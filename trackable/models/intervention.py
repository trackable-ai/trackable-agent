...
class InterventionType(StrEnum):
    ...
    DELIVERY_CONFIRMATION = "delivery_confirmation"  # Package delivered, return window starts
...
class RecommendedAction(BaseModel):
    ...
    deep_link: Optional[HttpUrl] = Field(default=None, description="Deep link to perform action")
    estimated_time_minutes: Optional[int] = Field(default=None, description="Estimated time to complete")
    deadline: Optional[datetime] = Field(default=None, description="Deadline to take this action")
    pre_drafted_message: Optional[str] = Field(default=None, description="Pre-drafted message for support contact")
...
class Intervention(BaseModel):
    ...
    recommended_actions: list[RecommendedAction] = Field(default_factory=list, description="Actions the agent recommends")

    # Context
    context: dict[str, Any] = Field(default_factory=dict, description="Additional context data")
    reasoning: Optional[str] = Field(default=None, description="Agent's reasoning for this intervention")

    # Timing
    triggered_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="When intervention was triggered")
    scheduled_for: Optional[datetime] = Field(default=None, description="When to deliver this intervention")
    sent_at: Optional[datetime] = Field(default=None, description="When intervention was sent")
    seen_at: Optional[datetime] = Field(default=None, description="When user saw it")
    acted_on_at: Optional[datetime] = Field(default=None, description="When user took action")

    # Delivery
    delivery_channels: list[str] = Field(default_factory=list, description="Channels used for delivery (email, push, etc.)")
    delivered: bool = Field(default=False, description="Whether intervention was delivered")

    # Metadata
    expires_at: Optional[datetime] = Field(default=None, description="When this intervention is no longer relevant")
