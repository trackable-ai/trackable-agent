...
class PolicyCondition(BaseModel):
    ...
    applies_to_categories: list[str] = Field(default_factory=list, description="Product categories this applies to")
    applies_to_price_range: Optional[tuple[float, float]] = Field(default=None, description="Price range (min, max)")
    exceptions: list[str] = Field(default_factory=list, description="Exceptions to this condition")


class ReturnPolicy(BaseModel):
    """Return policy details"""
    allowed: bool = Field(description="Whether returns are allowed")
    window_days: Optional[int] = Field(default=None, description="Return window in days from delivery")
    conditions: list[ReturnCondition] = Field(default_factory=list, description="Return conditions")
    refund_method: RefundMethod = Field(default=RefundMethod.UNKNOWN, description="Refund method")
    restocking_fee: Optional[float] = Field(default=None, description="Restocking fee percentage (0-100)")
    ...
    free_return_label: bool = Field(default=False, description="Merchant provides free return label")
    special_conditions: list[PolicyCondition] = Field(default_factory=list, description="Special conditions")
    excluded_categories: list[str] = Field(default_factory=list, description="Non-returnable categories")


class ExchangePolicy(BaseModel):
    """Exchange policy details"""
    allowed: bool = Field(description="Whether exchanges are allowed")
    window_days: Optional[int] = Field(default=None, description="Exchange window in days from delivery")
    exchange_types: list[ExchangeType] = Field(default_factory=list, description="Types of exchanges allowed")
    conditions: list[ReturnCondition] = Field(default_factory=list, description="Exchange conditions")
    shipping_responsibility: ReturnShippingResponsibility = Field(
        default=ReturnShippingResponsibility.UNKNOWN,
        description="Who pays for exchange shipping"
    )
    free_exchange_label: bool = Field(default=False, description="Merchant provides free exchange label")
    ...
    special_conditions: list[PolicyCondition] = Field(default_factory=list, description="Special conditions")
    excluded_categories: list[str] = Field(default_factory=list, description="Non-exchangeable categories")
...
class Policy(BaseModel):
    ...
    effective_date: Optional[datetime] = Field(default=None, description="When this policy became effective")

    # Return and exchange policies
    return_policy: Optional[ReturnPolicy] = Field(default=None, description="Return policy details")
    exchange_policy: Optional[ExchangePolicy] = Field(default=None, description="Exchange policy details")

    # Policy sources
    source_url: Optional[HttpUrl] = Field(default=None, description="URL to policy page")
    raw_text: Optional[str] = Field(default=None, description="Raw policy text as extracted")

    # Agent interpretation metadata
    confidence_score: Optional[float] = Field(default=None, ge=0.0, le=1.0, description="Policy interpretation confidence")
    last_verified: Optional[datetime] = Field(default=None, description="Last time policy was verified")
    needs_verification: bool = Field(default=False, description="Policy needs human verification")
    interpretation_notes: list[str] = Field(default_factory=list, description="Agent's interpretation notes")
        ...
        json_schema_extra = {
            "example": {
                ...
                "return_policy": {
                    ...
                    "excluded_categories": ["personalized items", "gift cards"]
                },
